...
class KenyansPipeline:
    ...
    def create_table(self):
        self.curr.execute("""DROP TABLE IF EXISTS  kenyans_tb """)
        self.curr.execute("""create table kenyans_tb (
    ...
    def store_db(self, item):
        ...
        if str(link) != "":
            ...
            w = str(x[1])
        sql = "INSERT IGNORE INTO kenyans_tb (links, title) VALUES (%s, %s)"
        val = (w, titlex)
...
class MytukonewsPipeline:
    ...
    def store_db(self, item):
        ...
        if str(link) != "":
            ...
            print("Value to be inserted is ===> ", str(x[1]))
            w = str(x[1])
        sql = "INSERT IGNORE INTO tuko_tb (links, title) VALUES (%s, %s)"
        val = (w, titlex)
        self.curr.execute(sql,val)
        self.conn.commit()
...
class StarPipeline:
    ...
    def store_db(self, item):
        ...
        if str(link) != "":
            ...
            print("Value to be inserted is ===> ", str(x[1]))
            w = str(x[1])
        sql = "INSERT IGNORE INTO star_tb (links, title) VALUES (%s, %s)"
        val = (w, titlex)
        self.curr.execute(sql,val)
        self.conn.commit()
